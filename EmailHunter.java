...
public class Email_Hunter_Edit { // We should refactor the Class name to
									// EmailFinder (Because that's the .java
									// file the AutoGrading system expects

	public static ArrayList<String> removehyperlink(ArrayList<String> s) {
		String str = " ";

		for (int i = 0; i < s.size(); i++) { // Converts the ArrayList into a
												// string with each element of
												// the ArrayList separated by a
												// Space.
			String temp = s.get(i);
			str += temp + " ";
		}
		...
		while (str.contains("\""))
			str = str.replaceAll("\"", " ");
		while (str.contains("NOSPAM"))
			str = str.replaceAll("NOSPAM", "");

		// System.out.println("After Clean: " + str);

		ArrayList<String> temp = new ArrayList<String>(); // Takes every element
															// of the new String
															// (which was the
															// original
															// ArrayList
															// concatenated with
		String[] elements = str.trim().split(" "); // spaces between each
													// element. Thus this makes
													// a String array and
													// ArrayList out of the
													// String
		// "temp" will now be the Original ArrayList, but without HTML
		// formatting symbols.
	...
	public static void main(String[] args) throws Exception {
		...
		while(whole.contains("(dot)")){
			System.out.println("i");
			whole = whole.replace("(dot)", ".");
		}
		while(whole.contains("(at)")){
			System.out.println("j");
			whole = whole.replace("(at)", "@");
		}
		while(whole.contains(" dot ")){
		...
		for (int i = 0; i < whole.length(); i++) {
			if (whole.charAt(i) == '@') {
				...
				if (whole.charAt(finish - 1) == '.') {
					while (!(emails
							.contains(whole.substring(start, finish - 1)))) // Removes
																			// the
																			// period
																			// if
																			// it
					...
					while (!(emails.contains(whole.substring(start, finish)))) // Leaves
																				// it
																				// otherwise
					{
		...
		for (int i = 0; i < emails.size(); i++) {
			...
			if (c2 == -1) {
				emails.remove(b);
				i--;
			} else if (end2.length() == 1 || end2.length() == 0) {
				emails.remove(b);
